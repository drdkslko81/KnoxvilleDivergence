...
def detect_knoxville_divergence(df, rsi_period=14, mom_period=12, lb=3):
    ...
    def is_pivot_high(series, i, window): 
        if i < window or i > len(series)-window-1: return False
...
with tab1:
    ...
    if st.button("🔍 Scan For Signals", type="primary"):
        ...
        for i, symbol in enumerate(symbols):
            try:
                data = fetch_data(symbol, "2025-01-01", datetime.now())
                if data is not None and len(data) > 100:
                    ...
                    if not signals_df.empty:
                        ...
                        # 🔥 DEBUG SECTION - Only show if toggle is ON
                        if show_debug:
                            with debug_container.container():
                                with st.expander(f"🔍 {symbol} Debug", expanded=False):
                                    st.json({
                                        'RSI': f"{float(last_row.get('RSI', 0)):.1f}",
                                        'Price_vs_EMA20': float(last_row.get('Close', 0)) > float(last_row.get('EMA20', 0)),
                                        'Signal_Found': last_row.get('Buy_Signal', 0) == 1 or last_row.get('Sell_Signal', 0) == 1,
                                        'Buy_Signal': last_row.get('Buy_Signal', 0),
                                        'Sell_Signal': last_row.get('Sell_Signal', 0),
                                        'Close_Price': f"₹{float(last_row.get('Close', 0)):.0f}",
                                        'RSI_Condition': float(last_row.get('RSI', 0)) < 50,
                        ...
                        if last_row['Buy_Signal'] == 1:
                            signals.append({
                                'Symbol': symbol,
                                'Signal': '🟢 BUY', 
                                'Price': f"₹{float(last_row['Close']):.0f}",
                                'RSI': f"{float(last_row['RSI']):.1f}",
                                'Date': last_row.name.strftime('%Y-%m-%d')
                            })
        ...
        if signals:
            ...
            st.info("📊 No fresh signals today. Check back tomorrow!")
            
            # Show summary stats if no signals
            if show_debug:
                st.info("🔍 **Debug Summary**: Enable debug toggle above to see why no signals generated.")
...
with tab2:
        ...
        with st.spinner("🔄 Running comprehensive backtest..."):
            ...
            for i, symbol in enumerate(symbols):
                try:
                    ...
                    # Extract signals with date + open price
    ...
    if st.session_state.backtest_data is not None:
        ...
        if not trades_df.empty:
            ...
            with col5:
                st.metric("Avg Holding", f"{avg_holding_days:.0f} days")  # NEW!
            ...
            **📊 Position Sizing & Holding Logic:**
            ...
            for col in ['Price', 'PnL', 'Total Capital', 'Value', 'Holding Days']:
                if col in page_trades.columns:
                    ...
                    # Color-code holding days (green=short, red=long)
                    if col == 'Holding Days':
